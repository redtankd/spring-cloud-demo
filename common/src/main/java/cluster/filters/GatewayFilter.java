...
public class GatewayFilter implements IgnitePredicate<ClusterNode>{
    /**
     * Checks if {@code node} needs to be considered as a Login Service Node.
     *
     * @param node Cluster node instance.
     *
     * @return {@code true} if the node has to be considered as Login Service Node, {@code false} otherwise.
     */
    public boolean apply(ClusterNode node) {
        Boolean dataNode = node.attribute("gateway.node");
